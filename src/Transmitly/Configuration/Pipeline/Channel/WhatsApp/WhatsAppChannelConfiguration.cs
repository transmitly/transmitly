...
sealed class WhatsAppChannelConfiguration(Func<IDispatchCommunicationContext, IPlatformIdentityAddress?>? fromAddressResolver = null) : IWhatsAppChannelConfiguration
{
	private readonly List<IWhatsAppContactConfiguration> _contacts = [];

	public IContentTemplateConfiguration Message { get; } = new ContentTemplateConfiguration();

	public IWhatsAppLocationConfiguration? Location { get; private set; }

	public IReadOnlyCollection<IWhatsAppContactConfiguration> Contacts => _contacts.AsReadOnly();

	public IWhatsAppTemplateConfiguration? Template { get; private set; }
	...
	public IWhatsAppChannelConfiguration AddLocation(Action<IWhatsAppLocationConfiguration> location)
	{
		Guard.AgainstNull(location);
		var config = Location as WhatsAppLocationConfiguration ?? new WhatsAppLocationConfiguration();
		location(config);
		Location = config;
		return this;
	}

	public IWhatsAppChannelConfiguration AddContact(Action<IWhatsAppContactConfiguration> contact)
	{
		Guard.AgainstNull(contact);
		var config = new WhatsAppContactConfiguration();
		contact(config);
		_contacts.Add(config);
		return this;
	}

	public IWhatsAppChannelConfiguration AddTemplate(Action<IWhatsAppTemplateConfiguration> template)
	{
		Guard.AgainstNull(template);
		var config = Template as WhatsAppTemplateConfiguration ?? new WhatsAppTemplateConfiguration();
		template(config);
		Template = config;
		return this;
	}
