...
namespace Transmitly.ChannelProvider.Tests
{
	...
	public class DeliveryReportMonitorTests
	{
		...
		public async Task ReportMonitorShouldFilterByEventName()
		{
			...
			monitor.OnNext(new DeliveryReport("test", null, null, null, null));
			monitor.OnNext(new DeliveryReport("test-2", null, null, null, null));
		...
		public async Task ReportMonitorShouldFilterByChannelId()
		{
			...
			monitor.OnNext(new DeliveryReport("test", "channel", null, null, null));
			monitor.OnNext(new DeliveryReport("test", "channel-2", null, null, null));
			await Task.Delay(10);//Delivery report calls are thrown out as tasks
			handler.Verify();
		}
		...
		public async Task ReportMonitorShouldFilterByChannelProviderId()
		{
			...
			monitor.OnNext(new DeliveryReport("test", "channel", "provider", null, null));
			monitor.OnNext(new DeliveryReport("test", "channel-2", "provider-2", null, null));
			await Task.Delay(10);//Delivery report calls are thrown out as tasks
			handler.Verify();
		}
		...
		public async Task ReportMonitorShouldFilterByAll()
		{
			...
			monitor.OnNext(new DeliveryReport("test", "channel", "provider", null, null));
			monitor.OnNext(new DeliveryReport("test-2", "channel-2", "provider-2", null, null));
			await Task.Delay(10);//Delivery report calls are thrown out as tasks
			handler.Verify();
		}
		...
		public async Task ReportMonitorShouldFireForAll()
		{
			...
			monitor.OnNext(new DeliveryReport("test", "channel", "provider", null, null));
			monitor.OnNext(new DeliveryReport("test-2", "channel-2", "provider-2", null, null));
			monitor.OnNext(new DeliveryReport("test-2", "channel-2", "provider-3", null, null));
			await Task.Delay(10);//Delivery report calls are thrown out as tasks
			handler.Verify();
		}
	}
